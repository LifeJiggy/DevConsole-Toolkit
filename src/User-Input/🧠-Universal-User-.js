...
(function () {
  ...
  const _HARDEN = {
    ...
    safe(fn, label) {
      try {
        ...
        console.warn(`[Harden] ${label} failed:`, e.message || e);
        return undefined;
      }
    },
    safeArr(fn, label) {
      const r = this.safe(fn, label);
      return Array.isArray(r) ? r : [];
    },
    domReady() {
      return !!(document && document.body && document.querySelectorAll);
    },
    queryAll(selector, limit) {
      if (!this.domReady()) return [];
      try {
        const els = document.querySelectorAll(selector);
        const max = limit || this.MAX_ELEMENTS;
        return Array.from(els).slice(0, max);
      } catch (_) {
        return [];
      }
    },
  ...
  function detectDangerousSinks() {
    return _HARDEN.safeArr(() => {
      if (!_HARDEN.domReady()) return [];
      console.log("sink === Detecting Dangerous DOM Sinks ===");
      const sinks = [];
      const sinkPatterns = [
        { name: "eval()", risk: "CRITICAL", src: /\beval\s*\(/i },
        { name: "Function()", risk: "CRITICAL", src: /\bFunction\s*\(/i },
        { name: "innerHTML=", risk: "HIGH", src: /\.innerHTML\s*=/i },
        { name: "outerHTML=", risk: "HIGH", src: /\.outerHTML\s*=/i },
        { name: "document.write()", risk: "HIGH", src: /document\.write\s*\(/i },
        { name: "document.writeln()", risk: "HIGH", src: /document\.writeln\s*\(/i },
        { name: "insertAdjacentHTML()", risk: "HIGH", src: /\.insertAdjacentHTML\s*\(/i },
        { name: "srcdoc=", risk: "HIGH", src: /\.srcdoc\s*=/i },
        { name: "javascript: href", risk: "CRITICAL", src: /\.href\s*=\s*['"]javascript:/i },
        { name: "setTimeout(string)", risk: "HIGH", src: /setTimeout\s*\(\s*['"]/i },
        { name: "setInterval(string)", risk: "HIGH", src: /setInterval\s*\(\s*['"]/i },
        { name: "dataset[]", risk: "MEDIUM", src: /\.dataset\s*\[/i },
        { name: "location=", risk: "MEDIUM", src: /window\.location\s*=/i },
        { name: "location.assign()", risk: "MEDIUM", src: /location\.assign\s*\(/i },
        { name: "location.replace()", risk: "MEDIUM", src: /location\.replace\s*\(/i },
        { name: "postMessage()", risk: "MEDIUM", src: /\.postMessage\s*\(/i },
      ];
      const scripts = _HARDEN.queryAll("script:not([src])", 200);
      scripts.forEach((scr) => {
        const code = scr.textContent || "";
        sinkPatterns.forEach(({ src, name, risk }) => {
          const re = _HARDEN.freshRegex(src);
          if (re.test(code)) {
            const matches = code.match(_HARDEN.freshRegex(src)) || [];
            sinks.push({ type: "script", sink: name, count: matches.length, risk });
          }
        });
      });
      const handlers = _HARDEN.queryAll("[onclick],[onload],[onerror],[onmouseover],[onfocus],[onblur],[onchange],[onsubmit],[oninput],[onkeydown],[onkeyup]", 500);
      handlers.forEach((el) => {
        Array.from(el.attributes).forEach((attr) => {
          if (/^on[a-z]+$/i.test(attr.name)) {
            const val = attr.value || "";
            sinkPatterns.forEach(({ src, name, risk }) => {
              if (_HARDEN.freshRegex(src).test(val)) {
  ...
  function detectSSRFPatterns() {
    return _HARDEN.safeArr(() => {
      ...
      const findings = [];
      const ssrfBase = /(?:https?:\/\/)?(?:localhost|127\.0\.0\.1|0\.0\.0\.0|10\.\d+\.\d+\.\d+|172\.(?:1[6-9]|2\d|3[01])\.\d+\.\d+|192\.168\.\d+\.\d+|169\.254\.\d+\.\d+|\[::1\]|metadata\.google\.internal|169\.254\.169\.254)/i;
      const allElements = _HARDEN.queryAll("input, textarea, a[href], form[action], script[src], img[src], iframe[src]", 3000);
      allElements.forEach((el) => {
        ...
        values.forEach((val) => {
          if (val && _HARDEN.freshRegex(ssrfBase).test(val)) {
      ...
      scripts.forEach((scr) => {
        const code = scr.textContent || "";
        const matches = code.match(_HARDEN.freshRegex(ssrfBase)) || [];
        matches.forEach((m) => {
  ...
  function detectCSRFProtection() {
    return _HARDEN.safeArr(() => {
      ...
      const results = [];
      _HARDEN.queryAll("form", 200).forEach((form) => {
        try {
  ...
  function analyzeTokenEntropy() {
    return _HARDEN.safeArr(() => {
      ...
      const findings = [];
      _HARDEN.queryAll('input[type="hidden"], input:not([type])', 1000).forEach((el) => {
        try {
          const val = el.value || "";
          if (val.length < 8) return;
          const uniqueChars = new Set(val).size;
          const safeLen = Math.min(val.length, 200);
          const entropy = uniqueChars > 0 ? Math.log2(Math.pow(uniqueChars, safeLen)) : 0;
          if (!isFinite(entropy)) return;
      ...
      _HARDEN.queryAll("form", 200).forEach((form) => {
        try {
          ...
          redirectParams.forEach((param) => {
            if (action.includes(param)) {
              findings.push({ type: "form-action", action: form.action.slice(0, 100), param, risk: "MEDIUM" });
            }
          });
        } catch (_) {}
      });
      _HARDEN.queryAll("a[href]", 2000).forEach((a) => {
        try {
          const href = (a.href || "").toLowerCase();
          redirectParams.forEach((param) => {
            if (href.includes(param + "=")) {
              findings.push({ type: "link", href: a.href.slice(0, 100), param, risk: "MEDIUM" });
            }
          });
        } catch (_) {}
      });
      _HARDEN.queryAll("script:not([src])", 200).forEach((scr) => {
        const code = scr.textContent || "";
        redirectParams.forEach((param) => {
  ...
  function detectDOMClobbering() {
    return _HARDEN.safeArr(() => {
      ...
      _HARDEN.queryAll("[id],[name]", 3000).forEach((el) => {
        try {
          const id = el.id || "";
          const name = el.name || "";
          [id, name].forEach((n) => {
      ...
      _HARDEN.queryAll("a[id], form[id], img[id]", 1000).forEach((el) => {
        try {
          const id = el.id || "";
  ...
  function detectPrototypePollution() {
    return _HARDEN.safeArr(() => {
      ...
      const findings = [];
      const patterns = [
        { src: /__proto__\s*\[/g, name: "__proto__[]", risk: "CRITICAL" },
        { src: /constructor\s*\[/g, name: "constructor[]", risk: "CRITICAL" },
        { src: /Object\.assign\s*\(/g, name: "Object.assign()", risk: "MEDIUM" },
        { src: /\.merge\s*\(/g, name: ".merge()", risk: "MEDIUM" },
        { src: /\.\.\.\s*\w+/g, name: "spread operator", risk: "LOW" },
        { src: /JSON\.parse\s*\(/g, name: "JSON.parse()", risk: "INFO" },
      ];
      _HARDEN.queryAll("script:not([src])", 200).forEach((scr) => {
        const code = scr.textContent || "";
        patterns.forEach(({ src, name, risk }) => {
          const re = _HARDEN.freshRegex(src);
      ...
      _HARDEN.queryAll("input, textarea", 1000).forEach((el) => {
        try {
          const val = el.value || "";
  ...
  function detectSanitizationFunctions() {
    return _HARDEN.safeArr(() => {
      ...
      const findings = [];
      const sanitizers = [
        { src: /\bDOMPurify\b/, name: "DOMPurify" },
        { src: /\bsanitize\s*\(/, name: "sanitize()" },
        { src: /\bescape\s*\(/, name: "escape()" },
        { src: /\bencodeURI\s*\(/, name: "encodeURI()" },
        { src: /\bencodeURIComponent\s*\(/, name: "encodeURIComponent()" },
        { src: /\bhtmlspecialchars\b/, name: "htmlspecialchars()" },
        { src: /\bhtmlEntities\b/, name: "htmlEntities()" },
        { src: /\bxssFilters?\b/, name: "xssFilter()" },
        { src: /\bpurify\b/i, name: "purify" },
        { src: /\bsanitizeHTML\b/, name: "sanitizeHTML()" },
        { src: /\bDOMPurify\.sanitize\b/, name: "DOMPurify.sanitize()" },
        { src: /\bcreatePolicy\b/, name: "Trusted Types createPolicy()" },
      ];
      _HARDEN.queryAll("script:not([src])", 200).forEach((scr) => {
        const code = scr.textContent || "";
        sanitizers.forEach(({ src, name }) => {
          if (_HARDEN.freshRegex(src).test(code)) {
            findings.push({ type: "script", sanitizer: name, risk: "INFO" });
          }
        });
      });
      _HARDEN.queryAll("script[src]", 500).forEach((scr) => {
        const src = scr.src || "";
  ...
  function discoverAPIEndpoints() {
    return _HARDEN.safeArr(() => {
      ...
      const endpoints = [];
      _HARDEN.queryAll("form[action]", 200).forEach((form) => {
        try {
      ...
      _HARDEN.queryAll("script:not([src])", 200).forEach((scr) => {
        const code = scr.textContent || "";
        let match;
        const re = _HARDEN.freshRegex(apiRegex);
        while ((match = re.exec(code)) !== null) {
          endpoints.push({ type: "js-fetch", url: match[1].slice(0, 100), risk: "INFO" });
        }
      });
      _HARDEN.queryAll("a[href^='http'], a[href^='/api'], a[href*='api.']", 2000).forEach((a) => {
        try {
          const href = a.href || "";
  ...
  function startDynamicContentTracker() {
    return _HARDEN.safe(() => {
      if (!_HARDEN.domReady()) { console.warn("tracker DOM not ready"); return; }
  ...
  function analyzeCSPDeep() {
    return _HARDEN.safe(() => {
      if (!_HARDEN.domReady()) return { headers: [], meta: [], issues: [], recommendations: [] };
      console.log("csp === Deep CSP Analysis ===");
      const report = { headers: [], meta: [], issues: [], recommendations: [] };
      const cspMeta = _HARDEN.queryAll('meta[http-equiv="Content-Security-Policy"]', 10).map((m) => m.getAttribute("content") || "");
      report.meta = cspMeta;
      if (cspMeta.length === 0) {
  ...
  function detectGraphQLEndpoints() {
    return _HARDEN.safeArr(() => {
      ...
      _HARDEN.queryAll("script:not([src])", 200).forEach((scr) => {
        const code = scr.textContent || "";
        if (_HARDEN.freshRegex(gqlRegex).test(code)) {
      ...
      _HARDEN.queryAll("script[src]", 500).forEach((scr) => {
        if (_HARDEN.freshRegex(gqlRegex).test(scr.src)) {
          findings.push({ type: "external-gql-script", src: scr.src.slice(0, 100), risk: "INFO" });
        }
      });
      _HARDEN.queryAll("[data-query], [data-mutation]", 500).forEach((el) => {
  ...
  function detectJWTExposure() {
    return _HARDEN.safeArr(() => {
      ...
      const findings = [];
      const jwtBase = /eyJ[A-Za-z0-9_-]{10,}\.eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]+/;
      _HARDEN.queryAll("input, textarea", 1000).forEach((el) => {
        try {
          const val = el.value || "";
          if (_HARDEN.freshRegex(jwtBase).test(val)) {
      ...
      _HARDEN.queryAll("script:not([src])", 200).forEach((scr) => {
        const code = scr.textContent || "";
        const jwtMatches = code.match(_HARDEN.freshRegex(jwtBase)) || [];
        jwtMatches.forEach(() => {
  ...
  function detectSSTIVectors() {
    return _HARDEN.safeArr(() => {
      ...
      const findings = [];
      const sstiPatterns = [
        { src: /\{\{.*\}\}/g, name: "Mustache/Handlebars" },
        { src: /\$\{.*\}/g, name: "EL/SpEL" },
        { src: /<%=.*%>/g, name: "ERB/EJS" },
        { src: /#\{.*\}/g, name: "Ruby interpolation" },
        { src: /\{%.*%\}/g, name: "Jinja2/Django" },
        { src: /\[\[.*\]\]/g, name: "Angular expression" },
      ];
      _HARDEN.queryAll("input, textarea", 1000).forEach((el) => {
        try {
          const val = el.value || "";
          sstiPatterns.forEach(({ src, name }) => {
            const re = _HARDEN.freshRegex(src);
            if (re.test(val)) {
